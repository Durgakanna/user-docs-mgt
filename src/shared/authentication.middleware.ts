...
export class AuthenticationMiddleware implements NestMiddleware{
    ...
    async use(request: Request, response: Response, next: NextFunction): Promise<void> {
       ...
       if (!accessToken) throw new HttpException("Access token is missing", HttpStatus.UNAUTHORIZED);
       try {
        const decoded = await this.authService.verifyToken(accessToken);
        if(!decoded)  throw new HttpException("Invalid or expired access token", HttpStatus.UNAUTHORIZED);
        next();
      } catch (error) {
